...
function initDiagram() {
    ...
    function configureChannel(e, obj) {
        ...
        var channel = obj.part.data.Channel ? obj.part.data.Channel : "";
        var msg_type = obj.part.data.MsgType ? obj.part.data.MessageType : "Bool";
        var rate = obj.part.data.Rate ? obj.part.data.Rate : "";
            
        var dialog = document.getElementById("channelConfigurationDialog")
        var btn_confirm = document.getElementById("channelConfigurationDialog_btn_confirm");
        var btn_cancel = document.getElementById("channelConfigurationDialog_btn_cancel");
        var select = document.getElementById("channelConfigurationDialog_select");
        var input = document.getElementById("channelConfigurationDialog_input");
        var input2 = document.getElementById("channelConfigurationDialog_input2");

        dialog.showModal();
        var prev_channel = channel;
        input.value = channel;
        input2.value = rate;
        var change = function(e2) {
            msg_type = select.value;
        }
        var confirm = function(e2) {
            channel = input.value;
            rate = input2.value;
            console.log(isChannelExisting(channel))
            if(prev_channel != channel && isChannelExisting(channel)) {
                alert('Already Exists');
                return;
            }
            myDiagram.model.setDataProperty(data, "Channel", channel);
            myDiagram.model.setDataProperty(data, "MessageType", msg_type);
            myDiagram.model.setDataProperty(data, "Rate", rate);
        ...
        dialog.addEventListener('close', function onClose() {
            ...
            btn_confirm.removeEventListener('click', confirm);
